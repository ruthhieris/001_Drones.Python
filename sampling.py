...
# In[46]:
    ...
    self.xmax = np.max(data[:, 0] + data[:, 3])
    
    self.ymin = np.min(data[:, 1] - data[:, 4])
    self.ymax = np.max(data[:, 1] + data[:, 4])
    
    self.zmin = 0
    # Limit the z axis for the visualization
    self.zmax = z_lim #np.max(data[:,2] + data[:,5] + 10) #10
